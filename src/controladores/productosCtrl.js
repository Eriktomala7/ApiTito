...
export const postProducto = async (req, res) => {
    try {
        const { prod_nombre, prod_codigo, prod_precio, prod_stock, prod_activo } = req.body;
        const prod_imagen_ruta = req.file ? `/uploads/${req.file.filename}` : null;
        const [result] = await conmysql.query(
            "INSERT INTO productos(prod_nombre, prod_codigo, prod_precio, prod_stock, prod_imagen, prod_activo) VALUES (?, ?, ?, ?, ?, ?)",
            [prod_nombre, prod_codigo, prod_precio, prod_stock, prod_imagen_ruta, prod_activo]
        );
        res.status(201).json({ prod_id: result.insertId });
    } catch (error) {
        return res.status(500).json({ message: "Error en el servidor", error: error.message });
